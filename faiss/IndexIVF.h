...
struct IndexIVF : Index, IndexIVFInterface {
    ...
    virtual IndexIVFStats get_ivf_stats() const;
...
struct IndexIVFStats {
    ...
    size_t c_list;             // nb of inverted lists scanned
    size_t ndis;              // nb of distances computed
    size_t nheap_updates;     // nb of times the heap was updated
    double quantization_time = 0.0f; // time spent quantizing vectors (in ms)
    double search_time = 0.0f;       // time spent searching lists (in ms)
    double compute_time = 0.0;
    double pruning_time = 0.0;
    double bound_time = 0.0;
    double bound_time_ip = 0.0;
    IndexIVFStats() {
        reset();
    }
    void reset();
