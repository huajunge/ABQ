 ...
 */
struct ABQ_PQFastScan  : IndexIVF {
...
//        std::mutex partition_mutex; // 分区级别的互斥锁
        ...
        std::vector<PQBlockIndex> blocks;
        std::pair<int, int> add(const float* x, const idx_t id, std::vector<uint8_t> codes,  const float* normalized_vector, float dist, float dist_c, float theta, size_t d);
    ...
    // 存储每个簇的分区数据
    std::vector<std::vector<PQPartition>> partitions_in_clusters;
...
//    void add_with_ids(idx_t n, const float* x, const idx_t* xids) override;
    ...
    ABQ_PQFastScan ();
    void split(
            std::vector<PQPartition >& partitions,
            PQPartition & partition,
    ...
    float angle_cos(
            const PQPartition & partition,
            const float* xi,
            float q_L2,
            float& d_q_p, bool load_ps = false) const noexcept;
};

struct ABQ_PQFastScanDedup: ABQ_PQFastScan  {
