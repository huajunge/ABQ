...
struct ABQSIMD : IndexIVF {
    ...
    // 每个簇内部"归一化垂向量两两 cos(phi)"的均值
    std::vector<float> cluster_max_cos_phi;
    // 每个簇内部"归一化垂向量两两 cos(phi)"的标准差
    std::vector<float> cluster_std_cos_phi;
    // 每个簇的垂向量均值方向（单位向量），长度为 d
    ...
    // ========== 方式2: z值幂律拟合参数 ==========
    // z(j) = a * j^(-b) + c + offset
    // 注意: 包含top1，从rank=1开始拟合，c + offset为常数，对应论文拟合的c值
    ...
    // 根据分位值获取对应的拟合参数 (a, b, c, offset)，c + offset为常数，对应论文拟合的c值
    // percentile: 75, 80, 85, 90, 95, 99, 999(99.9%)
    // 返回: z(j) = a * j^(-b) + c + offset
    ...
    // percentile: 75, 80, 85, 90, 95, 99, 999(99.9%)
    // cluster_rank: 簇排名（从1开始）
    // 公式: z(j) = a * j^(-b) + c + offset
    // 注意: 包含top1，从rank=1开始拟合
    // 特殊: 当percentile=99时，直接返回存储的P99原始分位值
...
//        std::mutex partition_mutex; // 分区级别的互斥锁
    ...
    // 存储每个簇的分区数据
    std::vector<std::vector<Partition>> partitions_in_clusters;
...
//    void add_with_ids(idx_t n, const float* x, const idx_t* xids) override;
    ...
    void refit_z_params(idx_t m, const float* queries);
