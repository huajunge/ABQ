...
//    all_ncentroids.push_back(5 * ncentroids);
    ...
    int train_vectors[] = {10000, 40000, 80000, 160000, 320000};
    int test_round = 2;
    int length = 5;
    ...
    std::string result_path_str = std::string(argv[4]) + dataset + "_" + index_type + "_training_data_.txt";
...
//        faiss::ABQ_SQ index(&coarse_quantizer, d, all_ncentroids[i]);
        ...
        { // training
            ...
            // 从database中取前train_vectors[i]个向量作为训练数据
            int n_train = train_vectors[i];
            // 确保不超过database的大小
            if (n_train > nb) {
                n_train = nb;
            }
            std::vector<float> sampled_train(n_train * d);

            printf("n_train: %d\n", n_train);
            float* tmp_database = database.get();
            std::copy(tmp_database, tmp_database + n_train * d, sampled_train.begin());

            index->train(n_train, sampled_train.data());
        }
...
//        all_nprobes.push_back((int)((float)ncentroids * (float)100.0f / 100.0));
        ...
        float* q = queries.get();
        nprobe = (int) ncentroids * (float)  5 / 100;
        for (int r = 0; r < test_round; r++) {
            printf("i: %d, r: %d, %s\n", i, r, index_type.c_str());
...
#pragma omp parallel for schedule(dynamic)
    ...
    printf("\n\ntrain_data\tQPS\trecall\n");

    for (size_t i = 0; i < length; ++i) {
        float qps = avg_qps[i];
        float recall = avg_recall[i];
        float ndis_v = avg_ndis[i];
        printf("%zu\t%f\t%f\t%f\n", train_vectors[i], qps, recall, ndis_v);
    }

    // 新增：将结果写入CSV文件
    // 第一行：训练数据量
    fprintf(result_file, "train_vectors");
    for (size_t i = 0; i < length; i++) {
        fprintf(result_file, "\t%d", train_vectors[i] );
    }
    fprintf(result_file, "\n");
