...
int main(int argc, char* argv[]) {
    ...
    int test_round = 2;
    int length = all_ncentroids.size();
    ...
    std::string result_path_str = std::string(argv[4]) + dataset + "_" + index_type + "_nlist_.txt";
...
//        faiss::ABQ_SQ index(&coarse_quantizer, d, all_ncentroids[i]);
        ...
        if (index_type == "ABQ") {
            index = std::make_unique<faiss::ABQSIMD>(&coarse_quantizer, d, all_ncentroids[i], block_distance);
        } else if (index_type == "ABQ_SQ") {
            index = std::make_unique<faiss::ABQ_SQ>(&coarse_quantizer, d, all_ncentroids[i], block_distance);
        } else if (index_type == "PQ") {
            index_name += std::to_string(all_ncentroids[i]);
            index_name += ",PQ";
        ...
        params.nprobe = all_ncentroids[i];
...
#pragma omp parallel for schedule(dynamic)
    ...
    for (size_t i = 0; i < length; ++i) {
        size_t nprobe = (i + 1) * std::sqrt(nb);
        float qps = avg_qps[i];
        float recall = avg_recall[i];
        float ndis_v = avg_ndis[i];
        printf("%zu\t%f\t%f\t%f\n", nprobe, qps, recall, ndis_v);
    }

    // 新增：将结果写入CSV文件
    // 第一行：nprobe占比
    fprintf(result_file, "nlist");
    for (size_t i = 0; i < length; i++) {
        fprintf(result_file, "\t%d", all_ncentroids[i]);
    }
    fprintf(result_file, "\n");
